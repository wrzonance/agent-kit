...
readonly CLOSING_REFERENCE_ATTEMPTS=3
...
verify_closing_reference() {
    ...
    local attempt=0
    while ((attempt < CLOSING_REFERENCE_ATTEMPTS)); do
        ((++attempt))
        # One page is not the whole edge set: a linkage sitting past the first
        # 100 would be reported as missing, so walk the cursor until the
        # expected issue is found or the pages genuinely run out.
        ...
        if ((attempt < CLOSING_REFERENCE_ATTEMPTS)); then
            sleep "$CLOSING_REFERENCE_RETRY_DELAY"
        fi
    done
