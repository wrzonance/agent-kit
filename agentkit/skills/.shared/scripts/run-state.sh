...
parse_args() {
    ...
    case $1 in
        get|set|append|append-unique|unset|latest|summary) ACTION=$1; shift ;;
...
resolve_summary_ledger() {
    ...
    primary_root=$(git -C "$checkout_root" worktree list --porcelain | sed -n 's/^worktree //p' | head -n 1)
...
print_summary() {
    ...
    counts=$(jq -er '
        ...
        positive_ids("opened_prs"; true) as $prs | positive_ids("queued"; true) as $queued |
        positive_ids("receipt_prs"; false) as $receipts | positive_ids("skipped_prs"; false) as $skipped |
...
main() {
    ...
    [[ ! -L $FILE ]] || die "state file must not be a symlink: $FILE"
    if [[ $ACTION == set || $ACTION == append || $ACTION == append-unique || $ACTION == unset ]]; then
