...
def verify(root, r, git):
    ...
    for v in r['verification']:
        ...
        observed.append([name,fingerprint,hashlib.sha256(data).hexdigest()])
    return observed
...
def validate(a,r,claims):
    ...
    failures=[]
    try:
        evidence=verify(root,r,git); claims['verification']='valid'
    ...
    try:
        ...
        remote=git('ls-remote','--refs','origin','refs/heads/'+r['branch']).decode().split() if 'origin' in remotes else []
        pushed=bool(remote and remote[0]==r['headSha'])
...
def store_receipt(a,receipt):
    ...
    key='results.'+a.attempt
    old=subprocess.run([str(HELPERS/'run-state.sh'),'get','--file',a.state,'--path',key],
                       capture_output=True,text=True,timeout=20)
    if old.returncode not in (0,11): raise Unknown('run-state receipt unavailable; preserve existing state')
    reused=old.returncode==0 and json.loads(old.stdout)==receipt and receipt['status']=='accepted'
    command([str(HELPERS/'run-state.sh'),'set','--file',a.state,'--path',key,'--json',json.dumps(receipt)])
    return reused
