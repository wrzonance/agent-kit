...
Agent-composed fenced recipes run in the harness shell, which may be zsh; a
`bash` label does not select Bash. Shipped `.sh` helpers use their shebang as the boundary.
...
Zsh's default `nomatch` makes an unmatched unquoted glob a hard error; quote globs passed as data.
Prefer `rg --glob '*.md'` when selecting paths without shell expansion.
Redirecting `ls … 2>/dev/null` is not enough because zsh rejects the glob before `ls` starts.
