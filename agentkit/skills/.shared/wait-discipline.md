...
Before dispatch, collection, or stopping, call `"$agentkit/.shared/scripts/run-state.sh" next-action --run-id "$RUN_ID" --repo-root "$repository_root" --worker-ledger "$worker_ledger" --dispatch-plan "$dispatch_plan" --json "$snapshot"` with fresh bound-run evidence, including live reviewer/test handles; after a steer add `--after-steer`. Both durable paths are required. The helper derives and saves queue, ownership, result, successor, PR, and receipt obligations; missing or ambiguous mappings remain reconciliation, never completion.
Every snapshot field is required; operation `affected` values refer to `remaining_work`, and actionable work is disjoint from active or unknown ownership. Follow `next_action`: dispatch independent work, collect active operations below, reconcile unknown/incomplete evidence through its existing bounded path, or complete normally. `outstanding` counts remaining obligations; `resume_required=true` continues this turn without replaying saved completed work.
Only `end-turn` or `complete` permits stopping. On `end-turn`, report completed work, each blocked workstream and exact question, remaining work, and saved progress, then stop immediately: `wait_allowed=false`, `task_complete=false`, and `ownership_released=false` forbid another wait, inventory query, or replacement worker merely to detect a reply. Incomplete or unknown evidence cannot select `end-turn`; a known independent actionable item still dispatches.
