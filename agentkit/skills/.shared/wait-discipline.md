...
Invoke `"$agentkit/.shared/scripts/run-state.sh" next-action --run-id "$RUN_ID" --repo-root "$repository_root" --json "$snapshot"`; after handling any operator message, add `--after-steer`. Its help names the required evidence, actionable work, typed operations, operator dependencies, completed work, and remaining work fields. `--after-steer` refuses exact replay of the saved evidence observation.
