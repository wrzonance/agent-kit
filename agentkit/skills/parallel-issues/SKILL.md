...
Immediately after establishing `RUN_ID`, run `"$agentkit/.shared/scripts/run-state.sh" init-summary --run-id "$RUN_ID" --repo-root "$repository_root"`; this creates only missing summary collections and preserves existing records on resume.
...
For every issue placed in either queue, record it once with `"$agentkit/.shared/scripts/run-state.sh" record-summary --run-id "$RUN_ID" --path queued --json "$issue"`.
...
- **Completion report (branch + pushed SHA)** → review the pushed diff, open the draft PR, record its numeric identity with `"$agentkit/.shared/scripts/run-state.sh" record-summary --run-id "$RUN_ID" --path opened_prs --json "$pr"`, move the issue to `In review`, and start Phase 3. Diff size is never a reason to withhold this PR — see Diff-size facts.
