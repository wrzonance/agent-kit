...
- `verification`: one object per root-required command: `command` is the declared command name shown
  as `cmd_name` in the composed verification runbook (the argument to `agent-run.sh --cmd`, matching
  `[a-z][a-z0-9-]*`, such as `test`, never the full command line); `status` (`pass`, `fail`,
