...
publish_rc=0
# --head-sha unlocks post-receipt.sh's own ledger write-back (issue #486 item 4).
rhs=$(gh api "repos/$REPO/pulls/$PR" --jq '.head.sha') || rhs=''
rla=(); [[ -z $rhs ]] || rla+=(--head-sha "$rhs")
