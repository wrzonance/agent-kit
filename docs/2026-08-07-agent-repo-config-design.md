...
~/.codex/skills/.shared/            <repo>/.agent/
  scripts/
    repo-config.sh    ──reads──▶      config.env    local, regenerable
    bootstrap-repo.sh ──writes─▶      board.json    local, regenerable
...
2. Parse **line-wise** into a whitelist of known keys. It must never `source` the
   file. Local state may be copied into logs or shared accidentally; treating it as
   executable shell would make it an injection vector into every agent's environment.
...
Generated by `bootstrap-repo.sh` as per-machine local state alongside `config.env`.
...
kept in the same local checkout whose board they describe. They carry no access
of their own.
...
- Records `.agent/*` in the repository-local `.git/info/exclude`; a fresh clone
  re-runs onboarding instead of inheriting declarations.
