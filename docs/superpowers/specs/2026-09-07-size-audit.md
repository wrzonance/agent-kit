...
| 4 | **D1-lite** `die`/`die_usage`/`die_evidence`/`require_value` into the lib the 16 review-remote-pr scripts already source (the full 46-script version nets −190 but adds 46 sibling-lib dependencies and an exit-code knob — the `code.md` "flags to cover its callers" smell; not recommended) | 16 + lib | 84 → 20 | **−64** | low |
...
Sum of the low-risk rows: **≈ −1,472 lines (4.6 % of the tree; 28 % of all comment lines)**; with the medium-risk rows ≈ −1,744 (recomputed 2026-09-08 alongside the row 1/row 4 corrections in `helpers-report.md`). No proposal adds a file (C2 forbids it), a gate, a rule, or a round trip; the only lib changes land in `lib/adversarial-review.sh` and `lib/private-dir.sh`, which already exist and are already sourced by the affected scripts.

Per-script maps for the five largest (function → LOC and top-3 cuts) are in `helpers-report.md §7`: agent-run ≈ −120, agent-preflight ≈ −260 (47 % comment), gh-pr-state ≈ −225, compose-worker-prompt ≈ −194, chain-advance ≈ −144.
