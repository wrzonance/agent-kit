...
declare -A KNOWN_OVERSIZE=(
    # LINES:TOKENS:TARGET
    [review-remote-pr]="497:7503:450"    # target <=450 lines; issue #107 phase 2 (WS5b) moved the Runtime-neutrality/environment-contract prose (references/environment-contract.md), the Implementation-worker-gate/publication-handback prose (references/worker-gate.md), and the Step 1b materiality criteria (references/adversarial-review.md) out of the body, ratcheting this from the last-committed 584:8931 (origin/feat/issue-106) down; the WS5b fix pass (adversarial finding: the paste-scope sentence wrongly told dispatchers to paste the whole dispatcher-side spawn-contract.md into a worker prompt) and a wave-3 accepted-review fix pass (corrected the worker-gate.md pointer's overclaim about what it carries) each nudged this back up to the ceiling above; it closes with this PR without hitting the target -- a successor issue is needed to track the remaining shrink work (see PR deviations), since most of what remains is executable recipe bytes pinned verbatim by test-review-artifacts.sh/test-parallel-dispatch-contract.sh/test-skills-contract.sh; test-skill-size.sh pins these exact ceiling numbers in its ratchet-message assertions, so a further ratchet needs a companion update to that test in the same PR
