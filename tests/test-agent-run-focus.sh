...
rc=0
out=$(cd "$missing" && "$run_sh" --cmd test --only unit 2>&1) || rc=$?
...
rc=0
out=$(cd "$runner_repo" && "$run_sh" --cmd test --only unit 2>&1) || rc=$?
...
# selector must fail before cache or execution.
rc=0
...
rc=0
out=$(cd "$repo" && "$run_sh" --only '' --cmd test 2>&1) || rc=$?
