...
for command in \
    ...
    case_id=$((case_id + 1))
    out=$(pre_input "$repo" "$command" "inline-$case_id" | "$hooks/pre-tool-use.sh" 2>/dev/null)
...
for command in \
    ...
    case_id=$((case_id + 1))
    out=$(pre_input "$repo" "$command" "file-$case_id" | "$hooks/pre-tool-use.sh" 2>/dev/null)
...
for command in \
    ...
    'echo "step 1; gh pr create --body body"' \
    ...
    case_id=$((case_id + 1))
    out=$(pre_input "$repo" "$command" "data-$case_id" | "$hooks/pre-tool-use.sh" 2>/dev/null)
    assert_eq '' "$(context "$out")" "quoted or argument-position gh stays quiet: $command"
done

# Separators in a quoted string and gh-looking text inside a heredoc body are
# data. Neither should consume the once-per-session advisory.
heredoc_command=$(printf '%s\n' "cat <<'EOF'" \
    'echo "step"; gh pr create --body body' 'EOF')
out=$(pre_input "$repo" "$heredoc_command" 'heredoc-body' |
    "$hooks/pre-tool-use.sh" 2>/dev/null)
assert_eq '' "$(context "$out")" 'heredoc body gh text stays quiet'
...
for command in \
    ...
    case_id=$((case_id + 1))
    out=$(pre_input "$repo" "$command" "separator-$case_id" | "$hooks/pre-tool-use.sh" 2>/dev/null)
...
out=$(pre_input "$repo" 'gh pr create --body "line\nnext"' 'backslash-n' | "$hooks/pre-tool-use.sh" 2>/dev/null)
...
out=$(pre_input "$repo" 'gh pr comment 1 --body body' 'comment-helper' |
    "$hooks/pre-tool-use.sh" 2>/dev/null)
