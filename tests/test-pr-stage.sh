...
mkdir -p "$tmp/bin" "$tmp/repo" "$tmp/run/state"
...
printf 'run-state %s %s\n' "$action" "$path" >>"$TEST_CALLS"
state=$TEST_STATE
[[ -e $state ]] || printf '{}\n' >"$state"
case $action in
    get)
...
common_env=(
    ...
    TEST_REPO="$tmp/repo" PR_STAGE_RUN_DIR_SH="$tmp/bin/run-dir"
...
rm -f "$tmp/state.json" "$tmp/register.marker"
: >"$tmp/calls"
register_rc=0
...
rm -f "$tmp/state.json" "$tmp/board.marker"
: >"$tmp/calls"
board_rc=0
...
rm -f "$tmp/state.json"
: >"$tmp/calls"
pr_list=$(jq -nc --rawfile body "$tmp/run/pr-stage-908-body.md" \
    '[{number:55,url:"https://github.com/owner/repo/pull/55",headRefName:"fix/issue-908",body:$body}]')
lost_output=$(env "${common_env[@]}" TEST_GH_BODY_MODE=lost TEST_PR_LIST="$pr_list" \
    "$helper" "${open_args[@]}")
...
assert_eq 1 "$(grep -c '^gh-lookup$' "$tmp/calls")" 'lost-response recovery makes one exact-head lookup'

rm -f "$tmp/state.json"
: >"$tmp/calls"
ambiguous=$(jq -nc --rawfile body "$tmp/run/pr-stage-908-body.md" \
    '[{number:61,url:"https://github.com/owner/repo/pull/61",headRefName:"fix/issue-908",body:$body},{number:62,url:"https://github.com/owner/repo/pull/62",headRefName:"fix/issue-908",body:$body}]')
ambiguous_rc=0
env "${common_env[@]}" TEST_GH_BODY_MODE=lost TEST_PR_LIST="$ambiguous" \
...
env "${common_env[@]}" "$helper" finalize --run-id wave \
    --repo-root "$tmp/repo" --pr 44 --repo owner/repo --agent-identity Codex \
...
: >"$tmp/calls"
final_output=$(env "${common_env[@]}" "$helper" finalize --run-id wave \
    --repo-root "$tmp/repo" --pr 44 --repo owner/repo --agent-identity Codex)
...
assert_eq $'gh-pr-state\npost-receipt publish\npost-receipt status\nrun-state record-summary receipt_prs' \
    "$(grep -E '^(gh-pr-state|post-receipt|run-state record-summary)' "$tmp/calls")" \
...
repeat_final=$(env "${common_env[@]}" "$helper" finalize --run-id wave \
    --repo-root "$tmp/repo" --pr 44 --repo owner/repo --agent-identity Codex)
...
rm -f "$tmp/state.json" "$tmp/summary.marker" "$tmp/post.marker"
: >"$tmp/calls"
summary_rc=0
env "${common_env[@]}" TEST_POST_ONCE=1 TEST_POST_MARKER="$tmp/post.marker" \
    ...
    "$helper" finalize --run-id wave --repo-root "$tmp/repo" --pr 44 --repo owner/repo \
