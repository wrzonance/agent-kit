...
# distinct exit-2 (declared-but-invalid) vs exit-1 (absent) so a malformed
...
assert_eq yes "$([[ $spawn_contract_bytes -le 18888 ]] && printf yes || printf no)" \
    "spawn contract stays at or under 18888 bytes including waiter caps (measured $spawn_contract_bytes)"
